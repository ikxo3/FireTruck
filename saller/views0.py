...
class AuthViewSet(viewsets.ModelViewSet):
    ...
    def login(self, request):
        ...
        GPIO.output(27, GPIO.LOW)   # green OFF
        GPIO.output(22, GPIO.LOW)   # blue OFF
        ...
        try:
            ...
            if user.expire_date < date.today():
                ...
                GPIO.output(22, GPIO.HIGH)  # blue ON (حساب منتهي)
                return Response(
            ...
            GPIO.output(17, GPIO.LOW)   # red OFF
            GPIO.output(22, GPIO.HIGH)  # blue ON (مستخدم غير موجود)
            return Response({"detail": "User not found"}, status=status.HTTP_404_NOT_FOUND)

        if not user.check_password(password):
            GPIO.output(17, GPIO.LOW)   # red OFF
            GPIO.output(22, GPIO.HIGH)  # blue ON (كلمة مرور خاطئة)
            return Response({"detail": "Invalid password"}, status=status.HTTP_400_BAD_REQUEST)

        # نجاح الدخول
        GPIO.output(17, GPIO.LOW)   # red OFF
        GPIO.output(27, GPIO.HIGH)  # green ON

        refresh = RefreshToken.for_user(user)
        return Response({
            'refresh': str(refresh),
            'access': str(refresh.access_token),
            'user': SallerSerializer(user).data
...
class SaleViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Sale.objects.filter(user =self.user)
